...
useEffect(()=>{
  ...
  return (
    <>
     <HeaderStudent answereddoubt={answereddoubt} pendingdoubt={pendingdoubt}/>
     ...
     <div className='w-full'>
       ...
       <div className='w-full px-5'>
          ...
          <Link key={e._id} to={{
            ...
            <li className='w-full bg-gray-300 hover:bg-gray-500 hover:text-white min-h-20 rounded-2xl list-none p-2 mt-5 shadow-sm shadow-black'>
              <div className='flex items-center justify-between'>
                <span><h1 className='ml-2 p-2'><b>Title:</b>{e.title}</h1></span>
                <span className='mr-5'>
                  <button onClick={(event)=>{event.preventDefault();event.stopPropagation();handleDelete(e._id);}} className='border border-black px-2 rounded hover:bg-red-700'>Delete</button>
                  </span>
              </div>
